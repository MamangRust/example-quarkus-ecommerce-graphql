    ...
    public Uni<ApiResponse<TokenResponse>> authenticate(LoginRequest loginRequest) {
        ...
        return userRepository.findByUsername(loginRequest.getUsernameOrEmail())
                ...
                .chain(user -> {
                    ...
                    String accessToken = jwtUtil.generateToken(user.getUsername(), roleNames, user.id);
                    String refreshToken = jwtUtil.generateRefreshToken(user.getUsername(), user.id);

                    logger.debug("Tokens generated for user: {}", user.getUsername());

                    return refreshTokenRepository.findByUserId(user.id)
                            .chain(refreshTokenModel -> {
                                ...
                                if (refreshTokenModel != null) {
                                    ...
                                    return refreshTokenRepository.deleteByUserId(user.id)
                                            .chain(v -> {
                    ...
                    .chain(refreshTokenEntity -> {
                        ...
                        return userRepository.findByUsername(username)
                                .chain(user -> {
                                    ...
                                    String newAccessToken = jwtUtil.generateToken(user.getUsername(), roles, user.id);
        ...
        return securityContext.getCurrentUser()
                .chain(user -> {
                    ...
                    if (cachedJson != null) {
                        ...
                        span.setAttribute("user.id", userResponse.getId().toString());
