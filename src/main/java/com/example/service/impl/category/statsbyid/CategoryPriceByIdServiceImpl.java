...
import com.example.repository.category.statsbyid.CategoryPriceByIdRepository;
...
public class CategoryPriceByIdServiceImpl implements CategoryPriceByIdService {
        ...
        public CategoryPriceByIdServiceImpl(CategoryPriceByIdRepository categoryPriceByIdRepository,
                ...
                this.tracer = openTelemetry.getTracer("category-price-by-id-stats-service", "1.0.0");
                Meter meter = openTelemetry.getMeter("category-price-by-id-stats-service");
        ...
        @Override
        public Uni<ApiResponse<List<CategoriesMonthPriceResponse>>> findMonthPriceById(MonthPriceIdRequest req) {
                ...
                String cacheKey = String.format("categories:stats:month-price-by-id:%d:%d", req.getCategoryId(),
                                req.getYear());
                ...
                return redisService.getReactive(cacheKey)
                                .chain(cachedJson -> {
                                        ...
                                        return categoryPriceByIdRepository
                                                        .findMonthlyCategoryStatsById(req.getCategoryId(),
                                                                        req.getYear())
                                                        .chain(results -> {
                                                                ...
                                                                return redisService
                                                                                ...
                                                                                .map(v -> {
                                                                                        logger.info("Cached monthly stats by category id for key: {}",
                                                                                                        cacheKey);
                                                                                        logger.info("Successfully fetched {} monthly stats for categoryId {}",
                                                                                                        responseList.size(),
                                                                                                        req.getCategoryId());
                                                        ...
                                                        .onFailure().invoke(e -> {
                                                                logger.error("Error fetching monthly price by category | CategoryId: {}, Year: {}",
                                                        ...
                                                        .eventually(() -> {
                                                                ...
                                                                requestDurationSeconds.record(duration, Attributes.of(
                                                                                AttributeKey.stringKey("operation"),
                                                                                "find_month_price_by_id"));
                                                                logger.debug("Fetch monthly price by category completed in {} seconds",
                                                                                duration);
                                                        });
                                });
        }
        ...
        @Override
        public Uni<ApiResponse<List<CategoriesYearPriceResponse>>> findYearPriceById(YearPriceIdRequest req) {
                if (req.getCategoryId() == null || req.getYear() == null) {
                        logger.error("❌ CategoryId or Year is null | req: {}", req);
                        throw new ResourceNotFoundException("CategoryId and Year must not be null");
                }

                String cacheKey = String.format("categories:stats:year-price-by-id:%d:%d", req.getCategoryId(),
                                req.getYear());
                ...
                return redisService.getReactive(cacheKey)
                                .chain(cachedJson -> {
                                        ...
                                        return categoryPriceByIdRepository
                                                        .findYearlyCategoryStatsById(req.getCategoryId(), req.getYear())
                                                        .chain(results -> {
                                                                ...
                                                                return redisService
                                                                                .setWithExpirationReactive(cacheKey,
                                                                                                ...
                                                                                                STATS_CACHE_TTL_SECONDS)
                                                                                .map(v -> {
                                                                                        logger.info("Cached yearly stats by category id for key: {}",
                                                                                                        cacheKey);
                                                                                        logger.info("Successfully fetched {} yearly stats for categoryId {}",
                                                                                                        responseList.size(),
                                                                                                        req.getCategoryId());
                                                                                        span.setStatus(StatusCode.OK);
                                                        ...
                                                        .onFailure().invoke(e -> {
                                                                logger.error("Error fetching yearly price by category | CategoryId: {}, Year: {}",
                                                                                req.getCategoryId(), req.getYear(), e);
                                                                span.recordException(e);
                                                                span.setStatus(StatusCode.ERROR, e.getMessage());
                                                        ...
                                                        .eventually(() -> {
                                                                ...
                                                                requestDurationSeconds.record(duration, Attributes.of(
                                                                                AttributeKey.stringKey("operation"),
                                                                                "find_year_price_by_id"));
                                                                logger.debug("Fetch yearly price by category completed in {} seconds",
                                                                                duration);
                                                        });
                                });
        }
