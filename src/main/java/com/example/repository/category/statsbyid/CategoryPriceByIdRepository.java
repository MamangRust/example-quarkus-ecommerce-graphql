package com.example.repository.category.statsbyid;
...
public class CategoryPriceByIdRepository implements PanacheRepository<Category> {
    ...
    public Uni<List<CategoriesMonthPrice>> findMonthlyCategoryStatsById(Integer categoryId, Integer year) {
        String sql = """
                WITH
                    date_range AS (
                        SELECT
    ...
    public Uni<List<CategoriesYearPrice>> findYearlyCategoryStatsById(Integer categoryId, Integer year) {
        String sql = """
                WITH last_five_years AS (
                    ...
                    GROUP BY c.id, c.name, EXTRACT(YEAR FROM o.created_at)
                )
                SELECT
                    "year" AS "year",
