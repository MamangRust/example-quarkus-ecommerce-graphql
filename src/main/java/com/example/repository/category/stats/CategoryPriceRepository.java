...
public class CategoryPriceRepository implements PanacheRepository<Category> {
    ...
    public Uni<List<CategoriesMonthPrice>> findMonthlyCategoryStats(Integer year) {
        String sql = """
                WITH date_range AS (
                    SELECT
                ...
                monthly_category_stats AS (
                    ...
                    GROUP BY
                        c.id, c.name, PARSEDATETIME(FORMATDATETIME(o.created_at, 'yyyy-MM-01'), 'yyyy-MM-dd')
                )
                SELECT
    ...
    public Uni<List<CategoriesYearPrice>> findYearlyCategoryStats(Integer year) {
        String sql = """
                WITH last_five_years AS (
                    ...
                    GROUP BY
                        c.id, c.name, EXTRACT(YEAR FROM o.created_at)
                )
                SELECT
                    "year" AS "year",
